...
const observer = new MutationObserver(function (list, self) {
    forEachArray(list, v => {
        ...
        if (TOKEN_ATTRIBUTES === type) {
            if (TOKEN_CLASS === attributeName) {
                if (hasClass(target, TOKEN_CLASS_HAS_ITEMS)) {
                    ...
                    let children = getChildren(target).filter(v => isElement(v) && (hasClass(v, TOKEN_CLASS_LINK) || hasClass(v, TOKEN_CLASS_LINK_SET)));
                    if (!toCount(children)) {
        ...
        if (TOKEN_CHILD_LIST === type) {
            ...
            let hasItems = toCount(getChildren(target).filter(v => isElement(v) && (hasClass(v, TOKEN_CLASS_LINK) || hasClass(v, TOKEN_CLASS_LINK_SET)))) > 0;
...
export function watchLinks(nodes) {
